...
function parseSkillLevels(entries, warnings, enkaAgentId) {
  ...
  for (const [index, category] of Object.entries(SKILL_INDEX_TO_CATEGORY)) {
    const level = boundedInteger(byIndex.get(index), 1, 16)
    if (level == null) {
...
function parseCoreSkill(value, warnings, enkaAgentId) {
  const index = boundedInteger(value, 1, CORE_SKILL_LEVELS.length)
  if (index == null) {
    warnings.push(`角色 ${enkaAgentId} 的核心技等级缺失或越界，保留 Calculator 原值。`)
    return null
  }
  return CORE_SKILL_LEVELS[index - 1]
}

function parseWEngine(weapon, warnings, enkaAgentId) {
  if (!weapon || weapon.Id == null) return null
  const enkaId = String(weapon.Id)
  const level = boundedInteger(weapon.Level, 1, 60)
...
function parseDriveDiscs(entries, warnings, enkaAgentId) {
  ...
  for (const entry of entries) {
    ...
    if (!slot || !equipmentId || !uid || level == null || !Array.isArray(mainEntries)
      ...
      warnings.push(`${context}结构无效或存在重复，保留 Calculator 原驱动盘预设。`)
      valid = false
      continue
    }
  ...
  if (!valid || driveDiscs.length !== entries.length) {
    warnings.push(`角色 ${enkaAgentId} 的驱动盘数据未完整解析，已保留 Calculator 原预设。`)
    return null
  }
...
export function parseEnkaShowcase(enkaData) {
  ...
  for (const avatar of extractAvatarList(enkaData)) {
    ...
    agents.push({
      ...
      skillLevels: parseSkillLevels(avatar.SkillLevelList, warnings, enkaId),
  ...
  if (!agents.length) {
    warnings.push("Enka 没有返回公开展柜角色，请确认已开启角色详情展示。")
  }
