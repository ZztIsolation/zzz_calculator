...
function changeKind(kind: string) {
  ...
  if (["direct", "sheer", "sharp"].includes(kind) && agentSkill()) {
    ...
    if (kind === "sharp") {
      props.event.sharpProfileId = props.agent?.sharpProfile?.id ?? "armorer"
      props.event.sharpComponent = "normal"
      props.event.sharpScenario = {
...
<template>
  <div class="calculation-event-grid">
    ...
    <template v-if="['direct', 'sheer', 'sharp'].includes(visibleKind())">
      ...
      <template v-if="visibleKind() === 'sharp'">
        <label class="maintenance-field"><span>锐化组件</span><NSelect v-model:value="event.sharpComponent" :options="[{ label: '普通锐化', value: 'normal' }, { label: '毁伤', value: 'maim' }]" :disabled="disabled" @update:value="emit('change')" /></label>
        <label v-if="event.sharpComponent === 'maim'" class="maintenance-field"><span>毁伤触发方式</span><NSelect v-model:value="event.maimTrigger" :options="[{ label: '消耗残痕', value: 'gash' }, { label: '六影免费毁伤', value: 'free' }]" :disabled="disabled" @update:value="emit('change')" /></label>
        <label class="maintenance-field"><span>猩红铭刻</span><NSwitch :value="event.sharpScenario?.crimsonInscription !== false" :disabled="disabled" @update:value="event.sharpScenario = { ...(event.sharpScenario ?? {}), crimsonInscription: Boolean($event) }; emit('change')" /></label>
        <label class="maintenance-field"><span>残痕层数</span><NInputNumber :value="event.sharpScenario?.gashStacks ?? 3" :disabled="disabled" :min="0" :max="3" :step="1" @update:value="event.sharpScenario = { ...(event.sharpScenario ?? {}), gashStacks: Number($event ?? 0) }; emit('change')" /></label>
        <label class="maintenance-field"><span>残锋</span><NSwitch :value="event.sharpScenario?.remnantEdgeActive !== false" :disabled="disabled" @update:value="event.sharpScenario = { ...(event.sharpScenario ?? {}), remnantEdgeActive: Boolean($event) }; emit('change')" /></label>
        <label class="maintenance-field"><span>音擎触发效果</span><NSwitch :value="event.sharpScenario?.triggeredEngineEffects !== false" :disabled="disabled" @update:value="event.sharpScenario = { ...(event.sharpScenario ?? {}), triggeredEngineEffects: Boolean($event) }; emit('change')" /></label>
      </template>
