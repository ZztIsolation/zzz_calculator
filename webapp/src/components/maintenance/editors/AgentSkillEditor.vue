        ...
        <article v-for="(move, moveIndex) in category.moves" :key="move.id" class="maintenance-subcard skill-move-card">
          ...
          <div class="skill-table-wrap">
            <table class="skill-multiplier-table">
              <thead><tr><th class="skill-col-label">行名</th><th class="skill-col-kind">类型</th><th class="skill-col-basis">伤害基准</th><th>次数范围</th><th v-for="level in levelsFor(category)" :key="level">{{ category.levelScale === 'coreSkill' ? level : `LV${level}` }}</th><th>操作</th></tr></thead>
              <tbody>
