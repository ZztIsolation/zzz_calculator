...
describe("MaintenanceView structured editor", () => {
  ...
  it("exposes formula result units and readable shared stack groups", async () => {
    ...
    await field(rule, "来源类型").find("select").setValue("inCombatStat")
    expect(field(rule, "局内来源属性").find("select").element.value).toBe("critRate")
    expect(field(rule, "增幅类型").find("select").attributes("disabled")).toBeDefined()
    expect(rule.text()).not.toContain("默认来源数值")
    await button(rule, "添加公式参数").trigger("click")
    let parameterName = field(rule, "参数名").find("input")
    await parameterName.setValue("threshold")
    expect(rule.text()).toContain("参数 threshold 默认值")
    expect(rule.findAll(".maintenance-field").some(item => item.find("span").text().trim() === "threshold 的 R1/R2/R3/R4/R5")).toBe(false)
