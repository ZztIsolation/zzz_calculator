...
从上次推送到 `main` 之后，当前工作区主要更新如下：

- 新增多账号支持：不同账号拥有独立的驱动盘仓库、套装预设、导入记录和首页选择状态。
- 新增直伤、异常伤害与紊乱伤害计算，并提供防御、抗性、穿透、抗性无视、异常精通、异常等级、异常增伤、异常暴击和最终伤害等白盒拆解行。
...
- 首页和优化器增加目标预设、防御力、元素抗性、技能倍率选择、异常事件和紊乱时间控制。
...
- 异常和紊乱伤害使用 `data/anomaly_effects.json` 中的数据倍率。
- 音擎改装等级只物化 Buff 数值，不改变 60 级基础攻击力或高级属性。
