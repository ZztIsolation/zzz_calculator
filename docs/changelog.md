...
All four Ultimates share the chain category level while retaining `ultimate` targeting. Mirage applies +40% CRIT DMG to stunned Ultimate events, Sunflare uses ordinary DMG Bonus, and left-branch Contamination plus right-branch Totalize add core-scaled ATK multipliers to their own moves exactly once. The four branch effects are unified under the Core Passive using the direct-damage snapshot assumptions; resources, timing and later events remain outside the model. Ultimate groups include one independently leveled Special Attack: Assault Directive; EX Special groups allow including or excluding it for cooldown accounting. M3/M5 defaults use final levels 14/16 without a second cinema bonus. Unrevealed M6 effects remain excluded with a visible notice.

Added optional `requirement.agentIds` validation, calculation filtering and maintenance editing for Sol Exuvia's Pyrois-only resistance ignore; its general Attack-agent CRIT Rate remains available. Skill-sourced Buffs can resolve existing core growth references consistently in calculation, picker and maintenance. Whitebox multiplier text now distinguishes base and added multipliers.

Validation: all root regression stages passed, including Pyrois formula/condition checks, compiled/dense/fixed scoring, real Node Worker optimization and 160 optimizer fuzz seeds; all 48 frontend suites / 610 tests passed. Production frontend and static Pages builds passed. An isolated browser verified four-branch configuration persistence, unified Core Passive Buff selection, M6 notice, 390px layout without horizontal overflow, zero console errors, and a real browser Worker Top 10 matching the Node oracle. The Pyrois modeling update was later merged into `main` and promoted to production. A follow-up Enka mapping correction added avatar ID `1551` for showcase imports and was verified on the public UID import route.
