...
All four Ultimates share the chain category level while retaining `ultimate` targeting. Mirage applies +40% CRIT DMG only to stunned Ultimate events. Sunflare uses ordinary DMG Bonus; left-branch Contamination and right-branch Totalize add core-scaled ATK multipliers to their own moves exactly once. Buff states do not simulate resources, timing or later events. Ultimate groups include one independently leveled Special Attack: Assault Directive; EX Special groups allow including or excluding it for cooldown accounting. Default optional conditions are off, with one non-stunned Celestial Light 3+4 example. M3/M5 defaults use final levels 14/16 without a second cinema bonus. Unrevealed M6 effects remain excluded with a visible notice.
