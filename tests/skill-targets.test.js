...
assert.equal(skillMultiplierTargetId({ agentSkillId: "claret", categoryId: "special", moveId: "special_slash_gold" }), "skill:claret:special:special_slash_gold")
assert.equal(skillMultiplierTargetId({ agentSkillId: "claret", categoryId: "special" }), "")
...
assert.equal(moveCount, 125)
assert.equal(rowCount, 256)
...
const guancheTargets = combatBuffCatalog.fieldBuffs
    ...
    .effects.flatMap(effect => effect.target?.skillTargets ?? [])
assert.deepEqual(guancheTargets, [{ kind: "skillType", skillType: "basic" }])
