...
function parseMoveRows(move) {
    ...
    for (const row of rows.values()) {
        ...
        if (row.kind === "damageMultiplier") {
            ...
            row.damageBasis = "def"
            if (row.id === "maim") {
                row.sharpComponent = "maim"
                row.maimTrigger = "gash"
            }
        }
    }
...
function sharpEvent(id, categoryId, moveId, rowId, extra = {}) {
    return {
        ...
        sharpComponent: rowId === "maim" ? "maim" : "normal",
        ...(rowId === "maim" ? { maimTrigger: "gash" } : {}),
        skillRef: {
            agentSkillId: "claret",
            categoryId,
            moveId,
...
function claretAgent(page) {
    ...
    const commonDescription = "官方动态 Wiki 数据：克拉蕾的招式伤害均为锐化伤害，以防御力为招式基底；锋御双重锐暴与状态参数由 core/sharpDamage.js 统一计算。"
    const sharpScenario = {
    ...
    const gashGroup = {
        ...
        events: [
            ...
            sharpEvent("gash_maim", "special", moveIds["特殊技：血华誓·斩金断铁"], "maim", { maimTrigger: "gash" }),
        ],
    }
    const c6Group = {
        ...
        events: [
            ...
            sharpEvent("free_maim", "special", moveIds["特殊技：血华誓·斩金断铁"], "maim", { maimTrigger: "free" }),
        ],
    }
        ...
        combatBuffs: {
            corePassive: {
                ...
                effects: [{
                    ...
                    value: 15,
                    mode: "flat",
                    valueSource: { kind: "corePassiveScaling", field: "sharpCritRatePct" },
                    target: {
                        kind: "sharp",
                        sharpScenarioAnyOf: [
                            { sharpScenario: { crimsonInscription: true } },
                            { skillTypes: ["chain", "ultimate"] },
                            { moveIds: ["assist_counter_iron", "assist_pure_melt", "assist_carve_form"] },
                        ],
                    },
                }],
                buffModifiers: [],
            ...
            additionalAbility: {
                ...
                effects: [{
                    ...
                    value: 25,
                    mode: "flat",
                    target: { kind: "sharp", sharpScenario: { remnantEdgeActive: true } },
                }],
                buffModifiers: [],
            },
            cinemaBuffs: [
                {
                    ...
                    effects: [{
                        id: "claret-c1-maim-multiplier",
                        type: "fixed",
                        stat: "sharpMaimMultiplier",
                        value: 30,
                        mode: "flat",
                        target: { kind: "sharp", sharpComponents: ["maim"] },
                    }],
                    buffModifiers: [],
                },
                {
        ...
        coreSkill: {
            ...
            corePassiveScaling: {
                ...
                levels: [
                    ...
                    [25, 40], [27.5, 45], [30, 50],
                ].map(([sharpCritRatePct, sharpBuildupEfficiencyPct], index) => ({
                    level: index + 1,
                    sharpCritRatePct,
            ...
            verification: {
                officialEntryPageId: Number(page.id),
                officialEntryVersion: String(page.version),
                coreValues: "Initial/A/B/C/D/E/F sharp crit rate and remnant buildup efficiency are 15/20, 17.5/25, 20/30, 22.5/35, 25/40, 27.5/45, 30/50 percent.",
            },
        },
        preferredDriveDiscs: {
    ...
    return {
        2188: {
            ...
            effects: [
                ...
                fixedRule("scarlet-electric-sharp-dmg", "electricSharpDmg", levels.scarletSharp, { kind: "sharp", sharpScenario: { triggeredEngineEffects: true } }),
            ],
        },
        2189: {
            ...
            effects: [
                fixedRule("marrow-overflow-rate", "sharpOverflowPerCritRate", levels.marrowPer, { kind: "sharp" }),
                fixedRule("marrow-overflow-cap", "sharpOverflowCap", levels.marrowCap, { kind: "sharp" }),
            ],
        },
        2190: {
            ...
            effects: [
                fixedRule("moon-basic-damage", "dmgBonus", levels.moonBasic, {
                    ...
                    sharpScenario: { triggeredEngineEffects: true },
                }),
            ],
        },
        2200: {
            ...
            effects: [
                ...
                fixedRule(
                    ...
                    "defPct",
                    levels.catDef,
                    { kind: "sharp", sharpScenario: { triggeredEngineEffects: true } },
                    { basis: "baseDef" },
                ),
            ],
        },
