...
/srv/zzz-download-origin/
  downloads/
    zzz-scanner/
      ...
      1.0.47/ZZZ-Scanner.Next-win-x64-fdd.zip
      1.0.47/ZZZ-Scanner.Next-win-x64-self-contained.zip
```
...
The current supported OCR runtime is ZZZ Scanner Next `1.0.47`, with Helper `1.3.1` and protocol v4. Supported systems are Windows 10 1809 (Build 17763) or newer x64 and Windows 11 x64, including N and LTSC editions. x86, ARM64, and Windows 7 are outside the current support commitment. Version 1.0.47 enforces strict single-row navigation, Safe + TextCore panel verification with a 120 ms floor, and explicit round-trip evidence for adjacent identical items while retaining structured ROI, acceptance-gate, stability, window, DPI, capture-backend, visual-profile, and lifecycle diagnostics. These fields may enter the privacy-limited summary above; screenshots and OCR text remain local.
