...
The current supported OCR runtime is ZZZ Scanner Next `1.0.45`, with Helper `1.3.1` and protocol v4. Supported systems are Windows 10 1809 (Build 17763) or newer x64 and Windows 11 x64, including N and LTSC editions. x86, ARM64, and Windows 7 are outside the current support commitment. Version 1.0.45 transactionally validates the first two Drive Discs before either enters OCR, searches the visible viewport for a distinct witness when their panels are indistinguishable, and preserves the structured ROI, acceptance-gate, stability, window, DPI, capture-backend, visual-profile, and lifecycle diagnostics. These fields may enter the privacy-limited summary above; screenshots and OCR text remain local.
